...
// Advance the physical elevator state by the control period
        elevator_tick(CONTROL_CYCLE_PERIOD_MS);

        // Update your logger to also print the new elevator angle!
        float current_elevator = elevator_get_current_deflection();
        // ... add this to log_entry_t and logger_write_entry
...
void app_main(void) {
    ...
    BaseType_t ret = xTaskCreatePinnedToCore(
        ...
        NULL,
        0
    );// pinned hal_innput_task to Core 0 to listen to UART
    if (ret != pdPASS) {
    ...
    ret = xTaskCreatePinnedToCore(
        ...
        NULL,
        1
    );//pinned hal_innput_task to Core 1 to run maths 
    if (ret != pdPASS) {
